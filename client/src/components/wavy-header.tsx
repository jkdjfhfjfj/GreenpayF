      ...
      <div className={`absolute top-0 left-0 right-0 flex items-center justify-between ${paddingMap[size]}`}>
        {onBack && (
          <motion.button
            whileTap={{ scale: 0.95 }}
            onClick={onBack}
            className="material-icons text-white mr-3 p-2 rounded-full hover:bg-white/20 transition-colors"
          >
            arrow_back
          </motion.button>
        )}
        {title && (
          <h1 className="text-white font-semibold drop-shadow-lg flex-1" 
